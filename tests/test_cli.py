...
class TestCLI:
    ...
    @patch("grm.cli.GitManager")
    def test_finish_command_checkout_to_develop_after_completion(self, mock_git_manager):
        ...
        assert ("main",) in [call[0] for call in checkout_calls]  # Checkout main for merge
        assert ("develop",) in [call[0] for call in checkout_calls]  # Merge back to develop
        assert checkout_calls[-1][0] == ("develop",)  # Final checkout should be develop
        assert "✓ Switched to develop branch" in result.output

    @patch("grm.cli.GitManager")
    def test_finish_command_checkout_to_integration_when_no_develop(self, mock_git_manager):
